function(setup_generic_new_soc_libraries)
    message(STATUS "Setting up New GENERIC SOC libraries...")
    ...
    if(GRAPHFLOW_X86_SUPPORT)
        ...
        if(EXISTS "${EXTRACT_MARKER}")
            ...
            message(STATUS "SOC New ver2 libraries need extraction from ${NEW_SOC_LIB_ARCHIVE} to ${SOC_GENERIC_LIB_DIR}")
            message(STATUS "SOC New ver2 WORKING_DIRECTORY on ${SOC_GENERIC_WORK_DIR} ")
        ...
        # 查找所有 SOC 库
        message(STATUS "Looking for generic New ver2 SOC libraries in ${SOC_GENERIC_LIB_DIR}")
      
        set(NEW_GENERIC_SOC_LIBRARIES)
        ## use for GENERIC  (systemc-2.3.1)
        set(SOC_GENERIC_LIB_NAMES
            AA Afifo aicpu_wrapper pem_accelerator aicpu AXI_STREAM_BUS ChiRingFabric common LpddrWrap lpddrsim PCIE libxml2.so.2.9.9 libxml2.so.2 libxml2.so  hscb
            DDR_Inf dramsim DSA DVPP_CA EslTop GENERIC_CHI_IF  L2Buf MATA mcu_wrapper mcu_loop
            model_api NcMpi NET_FASTMODEL NETWORK_ICL NIC ParallelScheduler  PMU POE CuberWrapper  stars stars_wrapper
            PowerModel qtest_api RDMA SCHE SDMAA SDMAM SLLC SMMU SoC systemc-2.3.1 TaskSched TgWrapper UB Utility zptwrapper
        )

        # use for generic : model/generic_soc/lib_v2
        foreach(lib_generic_name IN LISTS SOC_GENERIC_LIB_NAMES)
            ...
            if(${lib_generic_name}_lib)
                ...
                list(APPEND NEW_GENERIC_SOC_LIBRARIES ${${lib_generic_name}_lib})
            else()
        ...
        # GENERIC soc任何库
        list(LENGTH NEW_GENERIC_SOC_LIBRARIES NEW_GENERIC_SOC_LIBRARIES_COUNT)
        if(NEW_GENERIC_SOC_LIBRARIES_COUNT EQUAL 0)
            message(FATAL_ERROR "No SOC libraries found in ${SOC_GENERIC_LIB_DIR}")
        else()
            message(STATUS "Found ${NEW_GENERIC_SOC_LIBRARIES_COUNT} SOC libraries")
        endif()
        ...
        # 导出变量到父作用域
        set(SOC_NEW_GENERIC_LIB_NAMES_GLOBAL ${SOC_GENERIC_LIB_NAMES} PARENT_SCOPE)
        set(SOC_NEW_GENERIC_LIBS_GLOBAL ${NEW_GENERIC_SOC_LIBRARIES} PARENT_SCOPE)
        set(SOC_NEW_GENERIC_EXTRACT_TARGET_GLOBAL extract_generic_soc_libs PARENT_SCOPE)
...
function(link_soc_generic_libraries TARGET_NAME)
    if(GRAPHFLOW_X86_SUPPORT AND TARGET ${TARGET_NAME} AND TARGET soc_generic_libraries_interface)
        ...
        # 添加依赖关系
        add_dependencies(${TARGET_NAME} ${SOC_NEW_GENERIC_EXTRACT_TARGET_GLOBAL})
