...
def output_autogen():
    output('/* This file is autogenerated by decodetree.py.  */\n\n')
    output('#include "../../ISACommon/InstInfo.h"\n')
    output('namespace JCore {\n')
...
def whex(val):
    """Return a hex string for val padded for binarywidth"""
    global binarywidth
    return f'0x{val:0{binarywidth // 4}x}'
...
# extract函数用于解析二进制得到操作数
    def str_extract(self):
        global bitop_width
        #s = 's' if self.sign else ''
        #return f'DecodeUtiles::{s}extract{bitop_width}(bin, {self.pos}, {self.len})'
        if self.sign:
            return f'DecodeUtiles::Sextract{bitop_width}(bin, {self.pos}, {self.len})'
        else:
            return f'DecodeUtiles::Extract{bitop_width}(bin, {self.pos}, {self.len})'
    ...
    def str_extract(self):
        ...
        for f in reversed(self.subs):
            ...
            if pos == 0:
                ...
                ret = f'DecodeUtiles::Deposit({ret}, {pos}, {ext})'
            pos += f.len
        return ret
...
class FunctionField:
    ...
    def str_extract(self):
        return 'DecodeUtiles::' + self.func + '(' + self.base.str_extract() + ')'
...
class Format(General):
    ...
    def output_extract(self):
        output('static InstInfo* ', self.extract_name(), '(',
               self.base.struct_name(), ' *inst, ', binarytype, ' bin)\n{\n')
        for n, f in self.fields.items():
            output('    inst->', n, ' = ', f.str_extract(), ';\n')
        output('    return inst;\n')
        output('}\n\n')
# end Format
...
class Pattern(General):
    ...
    def output_code(self, i, extracted, outerbits, outermask):
        ...
        for n, f in self.fields.items():
            output(ind, 'inst->', n, ' = ', f.str_extract(), ';\n')
        if not extracted:
            output(ind, self.base.extract_name(),
                   '(inst, bin);\n')
        output(ind, 'inst->opcode =  Opcode::OP_', self.name, ';\n')
...
class MultiPattern(General):
    ...
    def prop_masks(self):
        ...
        fixedmask = binarymask
        undefmask = binarymask
    ...
    def output_code(self, i, extracted, outerbits, outermask):
        ...
        for p in self.pats:
            if outermask != p.fixedmask:
                ...
                output(ind, f'if ((bin & {whexC(innermask)}) == {whexC(innerbits)}) {{\n')
    ...
    def output_code(self, i, extracted, outerbits, outermask):
        ...
        if sh > 0:
            ...
            def str_switch(b, sh=sh):
                return f'(bin >> {sh}) & {b >> sh:#x}'

            def str_case(b, sh=sh):
                return hex(b >> sh)
        else:
            def str_switch(b):
                return f'bin & {whexC(b)}'
        ...
        for b, s in sorted(self.subs):
            ...
            s.output_code(i + 8, extracted, innerbits, innermask)
            output(ind, '        break;\n')
        output(ind, '}\n')
# end Tree
...
class ExcMultiPattern(MultiPattern):
    ...
    def __build_tree(pats, outerbits, outermask):
        ...
        innermask = ~outermask & binarymask
        for i in pats:
            innermask &= i.fixedmask

        if innermask == 0:
            # Edge condition: One pattern covers the entire binarymask
            if len(pats) == 1:
...
def parse_field(lineno, name, toks):
    ...
    for t in toks:
        ...
        po = int(subtoks[0])
        le = int(subtoks[1])
        if po + le > binarywidth:
            error(lineno, f'field {t} too large')
        f = Field(sign, po, le)
        subs.append(f)
        width += le

    if width > binarywidth:
...
def parse_generic(lineno, parent_pat, name, toks):
    ...
    for t in toks:
        ...
        # Otherwise, fieldname:fieldwidth
            ...
            if flen[0] == 's':
                ...
                flen = flen[1:]
            shift = int(flen, 10)
            if shift + width > binarywidth:
                error(lineno, f'field {fname} exceeds binarywidth')
            f = Field(sign, binarywidth - width - shift, shift)
    ...
    if variablewidth and width < binarywidth and width % 8 == 0:
        shift = binarywidth - width
        fixedbits <<= shift
        fixedmask <<= shift
        undefmask <<= shift
        undefmask |= (1 << shift) - 1
    ...
    #elif not (is_format and width == 0) and width != binarywidth:
...
#               error(lineno, f'field {f} not initialized')
    ...
    if not is_format:
        ...
        if allbits != binarymask:
            error(lineno, 'bits left unspecified ',
                  f'({whex(allbits ^ binarymask)})')
# end parse_general
...
class SizeTree:
    ...
    def output_code(self, i, extracted, outerbits, outermask):
        ...
        if extracted < self.width:
            output(ind, f'bin = {decode_function}_load_bytes',
                   f'(bin, {extracted // 8}, {self.width // 8});\n')
...
def build_size_tree(pats, width, outerbits, outermask):
    ...
    innermask = 0xff << (binarywidth - width)
    innermask &= ~outermask
    minwidth = None
    onewidth = True
    for i in pats:
...
def main():
    ...
    long_opts = ['decode=', 'translate=', 'output=', 'binarywidth=',
                 'static-decode=', 'varbinarywidth=']
    try:
    ...
    for o, a in opts:
        if o in ('-o', '--output'):
            ...
            translate_scope = ''
        elif o in ('-w', '--binarywidth', '--varbinarywidth'):
            if o == '--varbinarywidth':
                variablewidth = True
            binarywidth = int(a)
            if binarywidth == 16:
                ...
                binarytype = 'uint64_t'
                binarymask = 0xffffffffffff
                bitop_width = 48
            elif binarywidth == 64:
                binarytype = 'uint64_t'
                binarymask = 0xffffffffffffffff
                bitop_width = 64
            elif binarywidth != 32:
                error(0, 'cannot handle bin of width', binarywidth)
        else:
    ...
    output('InstInfo* ', decode_function,
           '(', binarytype, ' bin)\n{\n')
    ...
    if len(allpatterns) != 0:
        ...
        #output(i4, '} u;\n\n')
        output(i4, 'InstInfo *inst = new InstInfo;\n')
        toppat.output_code(4, False, 0, 0)

    output(i4, 'return inst;\n')
    output('}\n')
    output('} // namespace JCore\n')

    if variablewidth:
        output('\n', decode_scope, binarytype, ' ', decode_function,
               '_load(MInst *minst)\n{\n',
               '    ', binarytype, ' bin = 0;\n\n')
