...
void MtcLSUStats::report(const std::string& name)
{
    rpt->ReportTitle(name + " Statistics");
    rpt->ReportValAndPct("L2 Stall L1D Cycles And Percent", l2_stall_l1d_cycles, cycles);
    rpt->ReportVal("Load Lane Requests", total_load_request);
    rpt->ReportVal("Store Lane Requests", total_store_request);
    rpt->ReportVal("L1 miss Read Req Cnt", dcache_l2_readpkt_count);
    ...
    rpt->ReportValAndPct("  |-- Repeadted L1 Read Packets Filtered By MissQ",
        dcache_l2_missq_filt_count, dcache_l2_pkt_count);
    rpt->ReportValAndPct("    |-- Demand Packets Filtered By MissQ",
        dcache_l2_demand_missq_filt_count, dcache_l2_pkt_count);
    ...
    rpt->ReportVal("  |-- Load Repick Cnt", ldq_pick_reqs);
    ...
    rpt->ReportVal("Tile SCB Write stall", tileScbStallCycles);
    rpt->ReportVal("ldq full stall", ldqStallCycles);
    rpt->ReportVal("send memory req stall", sendMemReqStallCycles);
