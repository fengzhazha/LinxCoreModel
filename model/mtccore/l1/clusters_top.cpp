...
void MTC_L1Top::Reset(void)
{
    for (auto &c : clusters)
        c.Reset();
}

void MTC_L1Top::Build(void)
{
    ...
    for (uint32_t i = 0; i < clusters.size(); ++i) {
        ...
        clusters[i].tag_scalper_l1_q = tag_scalper_l1_array[i];
...
SimSys *MTC_L1Top::GetSim(void)
{
    return sim;
}

bool MTC_L1Top::dataAllow()
{
    for (auto &c : clusters) {
        if (!c.dataAllow()) {
            return false;
