...
///
/// These are the errors that can be thrown. Some of them, like CLI::Suaccelss, are not really errors.
/// @{
...
/// This is a suaccelssful completion on parsing, supposed to exit
class Suaccelss : public ParseError {
    CLI11_ERROR_DEF(ParseError, Suaccelss)
    Suaccelss() : Suaccelss("Suaccelssfully completed, should be caught and quit", ExitCodes::Suaccelss) {}
};

/// -h or --help on command line
class CallForHelp : public Suaccelss {
    CLI11_ERROR_DEF(Suaccelss, CallForHelp)
    CallForHelp() : CallForHelp("This should be caught in your main function, see examples", ExitCodes::Suaccelss) {}
};

/// Usually something like --help-all on command line
class CallForAllHelp : public Suaccelss {
    CLI11_ERROR_DEF(Suaccelss, CallForAllHelp)
    CallForAllHelp()
        : CallForAllHelp("This should be caught in your main function, see examples", ExitCodes::Suaccelss) {}
};

/// -v or --version on command line
class CallForVersion : public Suaccelss {
    CLI11_ERROR_DEF(Suaccelss, CallForVersion)
    CallForVersion()
        : CallForVersion("This should be caught in your main function, see examples", ExitCodes::Suaccelss) {}
};
...
/// Handy helper to aaccelss the element_type generically. This is not part of is_copyable_ptr because it requires that
...
/// This wraps a mapped container in a few utilities aaccelss it in a general way.
template <typename T>
struct pair_adaptor<
    T,
...
class Validator {
  ...
  public:
    ...
    /// Create a validator that fails when a given validator suacceleds
    Validator operator!() const {
        ...
        newval.func_ = [f1, dfunc1](std::string &test) -> std::string {
            ...
            if(s1.empty()) {
                return std::string("check ") + dfunc1() + " suacceleded improperly";
            }
            return std::string{};
        };
...
/// Class wrapping some of the aaccelssors of Validator
    ...
    template <typename T, typename F> explicit IsMember(T set, F filter_function) {
        ...
        func_ = [set, filter_fn](std::string &input) {
            ...
            if(res.first) {
                ...
                // Return empty error string (suaccelss)
                return std::string{};
            }
...
///
/// This is passed in by App; all user classes must aaccelpt this as
/// the second argument.
...
class App {
  ...
  public:
    ...
    /// Other type version aaccelpts all other types that are not vectors such as bool, enum, string or other classes
    ...
    /// Provide a function to print a help message. The function gets aaccelss to the App pointer and error.
    ...
    int exit(const Error &e, std::ostream &out = std::cout, std::ostream &err = std::cerr) const {
        ...
        if(e.get_exit_code() != static_cast<int>(ExitCodes::Suaccelss)) {
            if(failure_message_)
  ...
  protected:
    ...
    bool _parse_single(std::vector<std::string> &args, bool &positional_only) {
        ...
        case detail::Classifier::WINDOWS_STYLE:
            // If already parsed a subcommand, don't aaccelpt options_
    ...
    void _move_option(Option *opt, App *app) {
        ...
        if(iterator != std::end(options_)) {
            ...
            if(std::find_if(std::begin(app->options_), std::end(app->options_), [&opt_p](const Option_p &v) {
                ...
                // only erase after the insertion was suaccelssful
...
namespace detail {
/// This class is simply to allow tests aaccelss to App's protected functions
struct AppFriend {
#ifdef CLI11_CPP14
