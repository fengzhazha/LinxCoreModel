...
private:
    ...
    typedef std::shared_ptr<L1Entry> PtrL1Entry;
    typedef std::shared_ptr<L0Entry> PtrL0Entry;

    uint64_t region_size  = 0x200;
    // uint64_t region_size  = 0x400;
    ...
    float    THRE_MISS = 0.0;
    PFConfLevel confLevel;
    std::vector<StreamConf>  confArray;
...
public:
    ...
    PFConfOP getPFConfOP(float accuracy, float lateness, float pollution, float miss);
    std::string getStr(PFConfOP op);
    void setPFParam();
};
