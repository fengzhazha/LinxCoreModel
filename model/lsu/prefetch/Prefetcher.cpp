...
void Prefetcher::insert(MemReqBus &req) {
    if (!configs->pref_enable) return;
    if (verbose) {
        LOG_DETAIL_M(Unit::LSU, Stage::NA) << "[debug-pref]: @" << sim->getCycles() << " tpc 0x" << std::hex << req.tpc
...
void Prefetcher::Reset(void) {
    stride.Reset();
    stream.Reset();
}

void Prefetcher::Work(void) {
...
void Prefetcher::Xfer(void) {
    stride.Xfer();
    stream.Xfer();
}

SimSys* Prefetcher::GetSim(void) {
    return sim;
}

void Prefetcher::Build(void) {
...
void Prefetcher::pfl1(uint64_t addr, uint32_t type) const
{
    MemReqBus req;
    req.vld      = 1;
    req.is_load  = true;
    ...
    if (verbose) {
        LOG_DETAIL_M(Unit::LSU, Stage::NA) << "[Pref]: " << sim->getCycles()
            << ", PFL1 0x" << std::hex << req.addr << std::dec << ". type " << type;
    }
}

void Prefetcher::pfl2(uint64_t addr, uint32_t type)
{
    ...
    if (verbose) {
        LOG_DETAIL_M(Unit::LSU, Stage::NA) << "[Pref]: " << sim->getCycles()
            << ", PFL2 0x" << std::hex << req->addr << std::dec << ". type " << type;
    }
}

bool Prefetcher::PrefetchBuffer::insert_pfl1(uint64_t addr) {
    bool r;
    if (pfl1_set.count(addr) != 0) {
        // printf("[PrefBuf]: filter PFL1. addr 0x%lx\n", addr);
        r = false;
    } else {
...
bool Prefetcher::PrefetchBuffer::insert_pfl2(uint64_t addr) {
    bool r;
    if (pfl2_set.count(addr) != 0) {
        // printf("[PrefBuf]: filter PFL2. addr 0x%lx\n", addr);
        r = false;
    } else {
...
void Prefetcher::PrefetchBuffer::insert_addr(uint64_t addr_i) {
    ...
    if (addr_set.count(addr) == 0) {
        ...
        if (addr_fifo.size() == depth) {
            ...
            // printf("[PrefBuf]: addr_set. delete 0x%lx\n", f_addr);
        }
        count++;
        if (count == depth) {
            count = 0;
