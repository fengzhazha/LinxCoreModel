...
bool Stride::train(MemReqBus &req)
{
    if (stride_disable) return false;
    if (prefetcher->verbose) {
        LOG_INFO_M(Unit::LSU, Stage::NA) << "[Stride]: " << __func__ << ". tpc 0x" << hex << req.tpc
            << ", addr 0x" << req.addr;
    }
    bool filtered = false;
    bool hit  = false;
        ...
        if (p->pc == req.tpc) {
            ...
            switch (p->stage) {
              ...
              case 2:
                ...
                break;
              default:
                LOG_ERROR_M(Unit::LSU, Stage::NA) << __func__ << ": unexpected stage " << p->stage;
                ASSERT(0);
            }
            p->addr = req.addr;
            break;
    ...
    if (hit) {
        if (e && !same_line) {
            ...
            if (!filter_line(l1_filter, pfl1_addr) && prefetcher->verbose) {
                LOG_INFO_M(Unit::LSU, Stage::NA) << "[Stride]: PFL1 0x" << hex << pfl1_addr << ", E->depth "
                    << dec << e->depth;
            }
            if (!filter_line(l2_filter, pfl2_addr) && prefetcher->verbose) {
                LOG_INFO_M(Unit::LSU, Stage::NA) << "[Stride]: PFL2 0x" << hex << pfl2_addr << ", E->depth "
                    << dec << e->depth;
            }
...
bool Stride::filter_line(std::deque<uint64_t> &filter, uint64_t addr)
{
    bool r = true;
...
void Stride::Work(void)
{
}

void Stride::Xfer(void)
{
}

void Stride::feedback(PtrPrefFB fb)
{
    if (fb->bad) {
        pfl1_bad_count_n++;
    }
    if (prefetcher->verbose) {
        LOG_INFO_M(Unit::LSU, Stage::NA) << "[Stride]: bad 0x" << hex << fb->addr;
    }
}
