...
/* unaligned/endian-independent pointer aaccelss */
 ...
 *
 * Note there are small differences with the softmmu aaccelss API!
 *
 * type is:
 * (empty): integer aaccelss
 *   f    : float aaccelss
 *
 * sign is:
 ...
 *   le   : little endian
 *   te   : target endian
 * (except for byte aaccelsses, which have no endian infix).
 *
 * The target endian aaccelssors are obviously only available to source
 * files which are built per-target; they are defined in cpu-all.h.
 *
 * In all cases these functions take a host pointer.
 * For aaccelssors that take a guest address rather than a
 * host address, see the cpu_{ld,st}_* aaccelssors defined in
 * cpu_ldst.h.
 *
